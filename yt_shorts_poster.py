#!/usr/bin/env python3
"""
YouTube Shorts Auto-Poster — Independent video pipeline.
Scans videos_to_upload/, splits long videos, analyzes frames + audio,
generates captions, and uploads as YouTube Shorts.
...
IG_USERNAME = ""  # Not used for YouTube
...
if KEYRING_AVAILABLE:
    ...
    if _kr_pw:
        IG_PASSWORD = _kr_pw
if not IG_PASSWORD:
    IG_PASSWORD = ""  # Not used for YouTube

VIDEO_FOLDER_PATH = Path(os.getenv("VIDEO_FOLDER", "videos_to_upload"))
POSTED_SHORTS_DIR = Path(os.getenv("POSTED_SHORTS_DIR", "youtube_posted_archive"))
TEMP_SEGMENTS_DIR = Path(os.getenv("TEMP_SEGMENTS_DIR", "temp_segments"))
MAX_SHORT_DURATION = int(os.getenv("MAX_SHORT_DURATION_SECONDS", "60"))
MIN_SEGMENT_DURATION = int(os.getenv("MIN_SEGMENT_SECONDS", "30"))
SHORTS_LOG_PATH = Path(os.getenv("SHORTS_LOG", "posted_shorts.json"))
...
POSTED_SHORTS_TXT = Path(os.getenv("POSTED_SHORTS_TXT", "posted_shorts.txt"))
...
DRY_RUN = os.getenv("YT_DRY_RUN", "false").lower() == "true"
DRY_RUN_DIR = Path(__file__).parent / "YT_TEST_OUTPUT"
...
for _d in [VIDEO_FOLDER_PATH, POSTED_SHORTS_DIR, TEMP_SEGMENTS_DIR, EXPLICIT_VIDEOS_DIR, VIDEO_DUPLICATES_DIR]:
...
def analyze_frame_with_vision(frame_path):
    ...
    if resp.status_code!=200:
        log.error(f"  Ollama {resp.status_code}: {resp.text[:300]}")
        log.error(f"  img_size={len(image_b64)}, model={VISION_MODEL}, url={OLLAMA_URL}")
        return ""
...
def is_video_already_posted(video_path):
    ...
    if POSTED_SHORTS_TXT.exists():
        try:
            posted_names = POSTED_SHORTS_TXT.read_text().splitlines()
...
def log_to_posted_txt(video_name, details=""):
    ...
    if details:
        line += f" | {details}"
    try:
        with open(POSTED_SHORTS_TXT, "a") as f:
            f.write(line + "\n")
    except Exception as e:
        log.warning(f"Could not write to posted_shorts.txt: {e}")
...
def generate_reel_caption(description, segment_path, transcript_text=""):
    ...
    prompt = f"""Write a YouTube Shorts caption in a casual, outdoorsy voice — engaging but concise.
...
- ONE sentence MAX, ultra-concise (60 chars text + hashtags).
- End with one quick question about the activity.
- Put ALL hashtags at the END of the caption (not the start). Include #shorts plus 1-2 additional relevant hashtags. Total 2-3 tags. Format: caption text here. #shorts #tag1 #tag2 #tag3 #tag4
- MAX 100 characters TOTAL - must fit in YouTube title. No profanity. No quotation marks.
{teaser}
...
def calculate_segments(duration):
    if duration <= MAX_SHORT_DURATION:
        return 1, duration
    num_segments = math.ceil(duration / MAX_SHORT_DURATION)
...
def register_segments_in_log(video_path, segments, duration):
    ...
    save_reels_log(data)
    log.info(f"  Registered {len(segments)} segments in posted_shorts.json")
...
def get_youtube_client():
    ...
    if not creds or not creds.valid:
        if creds and creds.expired and creds.refresh_token:
            ...
            client_secrets = Path(__file__).parent / os.getenv("YT_CLIENT_SECRETS_FILE", "client_secrets.json")
...
def upload_short_to_youtube(video_path, caption):
    """Upload a video as a YouTube Short."""
    log.info(f"Uploading YouTube Short: {Path(video_path).name}")

    try:
        youtube = get_youtube_client()
    except Exception as e:
        log.error(f"YouTube auth failed: {e}")
        add_error(f"YouTube auth failed: {e}")
        return False
    ...
    try:
        ...
        title = caption.split(chr(10))[0][:100]
        description = caption
        ...
        media = MediaFileUpload(str(upload_video), chunksize=-1, resumable=True)
        ...
        while response is None:
            ...
            if status_obj:
                log.info(f"Upload progress: {int(status_obj.progress() * 100)}%")

        video_id = response.get("id")
        log.info(f"Successfully uploaded YouTube Short! Video ID: {video_id}")
        return True

    except ResumableUploadError as e:
        log.error(f"YouTube upload failed: {e}")
        add_error(f"YouTube upload failed: {e}")
        return False
    except Exception as e:
...
def register_single_video(video_path, duration):
    """Register a non-segmented video in posted_shorts.json."""
    data = get_reels_log()
    ...
    if video_key not in data.get("videos", {}):
        ...
        data["last_post_time"] = datetime.now().isoformat()
        save_reels_log(data)
        log.info(f"Registered single video in posted_shorts.json: {video_name}")
...
def archive_original_video(video_key):
    ...
    if not src.exists():
        ...
        return
    POSTED_SHORTS_DIR.mkdir(parents=True, exist_ok=True)
    dest = POSTED_SHORTS_DIR / src.name
    counter = 1
    while dest.exists():
        stem = src.stem
        suffix = src.suffix
        dest = POSTED_SHORTS_DIR / f"{stem}_{counter}{suffix}"
        counter += 1
    try:
...
def get_reels_log():
    if SHORTS_LOG_PATH.exists():
        try:
            return json.loads(SHORTS_LOG_PATH.read_text())
        except json.JSONDecodeError:
            log.warning("Corrupt reels log; starting fresh")
    return {"videos": {}, "last_post_time": None}

def save_reels_log(data):
    SHORTS_LOG_PATH.parent.mkdir(parents=True, exist_ok=True)
    SHORTS_LOG_PATH.write_text(json.dumps(data, indent=2))
...
def run_once():
    log.info("=" * 60)
    log.info("Starting YouTube Shorts auto-poster cycle")
    log.info("=" * 60)
    ...
    if current_segment is None:
        ...
        if not video_path:
            ...
            return None
        duration = get_video_duration(video_path)
        needs_split = duration > MAX_SHORT_DURATION
        ...
        if needs_split:
            ...
            log.info(f"Processing {video_path.name} as single Short")
        ...
        if success:
            ...
            if is_segment:
                ...
                log_to_posted_txt(current_segment.name,
                                  f"{seg_duration:.1f}s, single Short")
